...
public class MoreThanOneContact {
	public static void main(String[] args) {
		...
		List<String> multiContactStudents = students.stream()
		        .filter(s -> s.contact.size() > 1)
		        .map(s -> s.name)
		        .collect(Collectors.toList());

		System.out.println("Students with multiple numbers: " + multiContactStudents);
	}
}
